#!/usr/bin/env python3
"""Deterministically render the TG5050 OpenSCAD model into pf-hwprobe skins.
...
MODEL = HERE / "trimui-smart-pro-s.scad"
DESCRIPTOR = ROOT / "devices" / "a523" / "capabilities.toml"
SKIN_DIR = ROOT / "skins" / "a523"
...
def descriptor_display_rect() -> dict[str, int]:
    ...
    if len(screens) != 1 or "display_rect" not in screens[0]:
        raise RuntimeError("a523 descriptor must have one projected display_rect")
    return {
        key: int(value)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    mode.add_argument(
        "--write",
        action="store_true",
        help="regenerate committed a523 body/body_lit assets and metadata",
    )
    mode.add_argument(
        "--check",
        action="store_true",
