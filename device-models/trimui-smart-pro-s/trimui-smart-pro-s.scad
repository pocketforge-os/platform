/*
 * PocketForge semantic device model — TrimUI Smart Pro S (TG5050)
 *
 ...
 * devices/a523/capabilities.toml [skin.parts] exactly, so the OpenSCAD source
...
// The owner's TG5050 cradle measurements match the accepted TG5040 envelope.
device_width = 188.35;              // owner calipers (shared chassis)
device_height = 79.77;              // owner calipers (shared chassis)
...
shoulder_span = 32.0;
power_centre_x = 0.30 * device_width;
home_centre_x = 0.39 * device_width;
...
assert(abs(device_width - 188.35) < 0.001 &&
       ...
       "TG5050 owner-caliper front envelope changed");
...
// TG5050 adds active cooling. Public multi-view photography resolves a
// two-row exhaust near the left shoulder and a three-disc striped fan intake
// on the rear shell. These remain visual/photo-derived rather than airflow
// engineering geometry.
function rear_exhaust_positions() = [
    for (row = [0 : 1], column = [0 : 12])
        [device_width -
             (38.0 + column * 1.55 + (row == 0 ? 0.0 : 0.78)),
         66.5 + row * 1.45]
];

function rear_fan_vents() = [
    [[device_width - 57.0, 43.0], -55],
    [[device_width - 49.5, 30.0], 0],
    [[device_width - 64.5, 30.0], 55]
];

module striped_disc_2d(diameter, angle) {
    intersection() {
        circle(d = diameter, $fn = 52);
        rotate(angle)
            union()
                for (offset = [-6.0 : 2.05 : 6.0])
                    translate([0, offset])
                        square([diameter + 2.0, 0.82], center = true);
    }
}

module rear_vent_recesses() {
    for (point = rear_exhaust_positions())
        translate([point.x, point.y, -epsilon])
            cylinder(d = 1.10, h = 0.30, $fn = 6);

    for (vent = rear_fan_vents())
        translate([vent[0].x, vent[0].y, -epsilon])
            linear_extrude(height = 0.30)
                striped_disc_2d(15.0, vent[1]);
}

module rear_vent_floors() {
    color([0.012, 0.015, 0.018, 1.0]) {
        for (point = rear_exhaust_positions())
            translate([point.x, point.y, 0.255])
                cylinder(d = 1.0, h = 0.025, $fn = 6);

        for (vent = rear_fan_vents())
            translate([vent[0].x, vent[0].y, 0.255])
...
module rear_details() {
    ...
    if (SHOW_MICRO_DETAILS)
        // The fixed rear camera mirrors model X. Source positions therefore
        // look reversed here while the rendered block sits right of the fan,
        // matching the TG5050 rear elevation.
        color(legend_color)
            translate([device_centre.x - 44, 36.0, -0.03])
                rotate([0, 180, 0])
                    linear_extrude(height = 0.10) {
                        translate([0, 5.0])
                            text("TRIMUI SMART PRO S", size = 1.35,
                                 halign = "center", valign = "center",
                                 font = "Liberation Sans:style=Bold");
                        translate([0, 1.8])
                            text("MODEL: TG5050  TRIMUI SMART PRO S", size = 1.05,
                                 halign = "center", valign = "center",
                                 font = "Liberation Sans:style=Bold");
                        translate([0, -1.0])
                            text("DC 5V / 1950mA", size = 1.05,
                                 halign = "center", valign = "center",
                                 font = "Liberation Sans:style=Bold");
                        translate([0, -5.0])
                            text("CE   FCC", size = 2.0,
                                 halign = "center", valign = "center",
                                 font = "Liberation Sans:style=Bold");
                    }
}
...
module home_button_control() {
    id = "btn_home";
    top_y = device_height - 0.15;

    // The Pro S addition is a small round top-edge key between POWER and HOST.
