...
Usage:
  gen_evdev_probe_codes.py --platform <dir>            # patch evdev-probe.py in place
                                                       # + rewrite evdev-dump-codes.h
  gen_evdev_probe_codes.py --platform <dir> --check    # assert no drift (CI)

The same tables are ALSO emitted as `evdev-dump-codes.h` for evdev-dump.c (the static-C
fallback dumper for stock userlands without python) — one generator, two consumers, so
the two capture tools can never disagree on a name.
"""
import argparse
...
C_HEADER_PATH = os.path.join(HERE, "evdev-dump-codes.h")


def _emit_c_header(tables):
    def one(name, d):
        # PF_ prefix: plain KEY_TAB would collide with the kernel's Tab-key #define.
        rows = "\n".join(f"    {{ {code:#x}, \"{d[code]}\" }},"
                         for code in sorted(d))
        return (f"static const struct code_name PF_{name}_TAB[] = {{\n{rows}\n}};\n"
                f"#define PF_{name}_TAB_LEN (sizeof(PF_{name}_TAB) / sizeof(PF_{name}_TAB[0]))\n")
...
def main():
    ...
    tables = _build_tables(a.platform)
    block = _emit_block(tables)
    with open(PROBE_PATH) as f:
        current = f.read()
    updated = _splice(current, block)

    c_header = _emit_c_header(tables)
    try:
    ...
    if a.check:
        ...
        if c_current != c_header:
            sys.exit(f"FAIL: {C_HEADER_PATH} is STALE vs kernel headers + caps.py vocab.\n"
                     f"      Regenerate: python3 {os.path.relpath(__file__)} --platform {a.platform}")
        print("OK    evdev-probe.py reverse tables match kernel ABI + caps.py vocab")
        print("OK    evdev-dump-codes.h matches kernel ABI + caps.py vocab")
        return 0
